...
class CommentController {
    async createComment(req, res) {
        try {
            ...
            const newComment = await db.query(
                `INSERT INTO PostComment (post_id, author_id, content) VALUES ($1, $2, $3) RETURNING *`,
    ...
    async getComments(req, res) {
        try {
            ...
            const comments = await db.query(
                `SELECT c.*, u.username FROM PostComment c JOIN "User" u ON c.author_id = u.id WHERE c.post_id = $1 ORDER BY c.created_at ASC`,
                [postId]
            );
    ...
    async deleteComment(req, res) {
        try {
            ...
            const comment = await db.query('SELECT id, author_id FROM PostComment WHERE id = $1 AND post_id = $2', [commentId, postId]);
            ...
            await db.query('DELETE FROM PostComment WHERE id = $1', [commentId]);
